...
{
	AddProperty(u"Version", u"Версия", [&](VH var)
							{ var = this->version(); });
}
...
long AddInNative::FindProp(const WCHAR_T *wsPropName)
{
	...
	for (auto it = properties.begin(); it != properties.end(); ++it)
	{
		...
		{
			if (n->compare(name) == 0)
				return long(it - properties.begin());
		}
	}
	name = upper(name);
	for (auto it = properties.begin(); it != properties.end(); ++it)
...
long AddInNative::GetNParams(const long lMethodNum)
{
	...
	if (it == methods.end())
		return 0;
	if (std::get_if<MethFunction0>(&it->handler))
		return 0;
	if (std::get_if<MethFunction1>(&it->handler))
		return 1;
	if (std::get_if<MethFunction2>(&it->handler))
		return 2;
	if (std::get_if<MethFunction3>(&it->handler))
		return 3;
	if (std::get_if<MethFunction4>(&it->handler))
		return 4;
	if (std::get_if<MethFunction5>(&it->handler))
		return 5;
	if (std::get_if<MethFunction6>(&it->handler))
		return 6;
	if (std::get_if<MethFunction7>(&it->handler))
		return 7;
	if (std::get_if<MethFunction8>(&it->handler))
		return 8;
	if (std::get_if<MethFunction9>(&it->handler))
		return 9;
	if (std::get_if<MethFunction10>(&it->handler))
		return 10;
	return 0;
}
...
bool AddInNative::CallMethod(MethFunction *func, tVariant *p, Meth *m, const long lSizeArray)
{
	...
	if (auto handler = std::get_if<MethFunction8>(func))
	{
		if (lSizeArray < 8)
			throw std::bad_function_call();
		(*handler)(VA(p, m, 0), VA(p, m, 1), VA(p, m, 2), VA(p, m, 3), VA(p, m, 4), VA(p, m, 5), VA(p, m, 6), VA(p, m, 7));
		return true;
	}
	if (auto handler = std::get_if<MethFunction9>(func))
	{
		if (lSizeArray < 9)
			throw std::bad_function_call();
		(*handler)(VA(p, m, 0), VA(p, m, 1), VA(p, m, 2), VA(p, m, 3), VA(p, m, 4), VA(p, m, 5), VA(p, m, 6), VA(p, m, 7), VA(p, m, 8));
		return true;
	}
	if (auto handler = std::get_if<MethFunction10>(func))
	{
		if (lSizeArray < 10)
			throw std::bad_function_call();
		(*handler)(VA(p, m, 0), VA(p, m, 1), VA(p, m, 2), VA(p, m, 3), VA(p, m, 4), VA(p, m, 5), VA(p, m, 6), VA(p, m, 7), VA(p, m, 8), VA(p, m, 9));
		return true;
	}
	return false;
}
...
std::u16string AddInNative::upper(std::u16string &str)
{
	std::transform(str.begin(), str.end(), str.begin(), [](wchar_t ch)
								 { return std::toupper(ch, locale_ru); });
	return str;
}
...
uint32_t AddInNative::VarinantHelper::size()
{
	if (pvar == nullptr)
		throw std::bad_variant_access();
	if (pvar->vt != VTYPE_BLOB)
		throw error(VTYPE_BLOB);
	return pvar->strLen;
}
...
AddInNative::VarinantHelper &AddInNative::VarinantHelper::operator=(const std::u16string &str)
{
	...
	while (pvar->wstrLen && pvar->pwstrVal[pvar->wstrLen - 1] == 0)
		pvar->wstrLen--;
	return *this;
}

AddInNative::VarinantHelper &AddInNative::VarinantHelper::operator=(std::vector<char> blob)
{
	clear();
	TV_VT(pvar) = VTYPE_BLOB;
	pvar->pstrVal = nullptr;
	size_t size = blob.size() * sizeof(char);
	if (!addin->AllocMemory(reinterpret_cast<void **>(&pvar->pstrVal), size))
		throw std::bad_alloc();
	memcpy(pvar->pstrVal, &blob.front(), size);
	pvar->strLen = blob.size();
	return *this;
}
...
bool AddInNative::AddError(const std::u16string &descr, long scode)
{
	...
	return m_iConnect && m_iConnect->AddError(ADDIN_E_INFO, (WCHAR_T *)info.c_str(), (WCHAR_T *)descr.c_str(), scode);
}
...
std::exception AddInNative::VarinantHelper::error(TYPEVAR vt) const
{
	...
	if (addin && addin->alias)
	{
		...
		if (prop)
			ss << u" при обращении к свойству <" << prop->names[1] << ">";
		if (meth)
			ss << u" при вызове метода <" << meth->names[1] << ">";
		if (number >= 0)
			ss << u" параметр <" << number + 1 << ">";
		ss << u" ожидается <" + typeinfo(vt, true) << u">";
		if (pvar)
			ss << u" фактически <" + typeinfo(pvar->vt, true) << u">";
	}
	else
	{
		ss << u"Error getting value";
		if (prop)
			ss << u" of property <" << prop->names[0] << ">";
		if (meth)
			ss << u" when calling method <" << meth->names[0] << ">";
		if (number >= 0)
			ss << u" parameter <" << number + 1 << ">";
		ss << u" expected <" + typeinfo(vt, false) << u">";
		if (pvar)
			ss << u" actual value <" + typeinfo(pvar->vt, false) << u">";
	}
	if (addin)
		addin->AddError(ss.str());
...
AddInNative::VarinantHelper::operator std::u16string() const
{
	if (pvar == nullptr)
		throw std::bad_variant_access();
	if (pvar->vt != VTYPE_PWSTR)
		throw error(VTYPE_PWSTR);
...
AddInNative::VarinantHelper::operator std::vector<char>() const
{
	if (pvar == nullptr)
		throw std::bad_variant_access();
	switch (TV_VT(pvar))
	{
	case VTYPE_BLOB:
		return std::vector<char>((char *)pvar->pstrVal, (char *)pvar->pstrVal + pvar->strLen);
	default:
		throw error(TV_VT(pvar));
	}
}
...
{
	clear();
	if (!addin->AllocMemory((void **)&pvar->pstrVal, size))
		throw std::bad_alloc();
	TV_VT(pvar) = VTYPE_BLOB;
	pvar->strLen = size;
}
