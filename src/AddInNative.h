...
protected:
	...
	public:
		...
		VarinantHelper &operator<<(const VarinantHelper &va)
		{
			pvar = va.pvar;
			addin = va.addin;
			prop = va.prop, meth = va.meth, number = va.number;
			return *this;
		}
		VarinantHelper &operator=(const VarinantHelper &va) = delete;
		VarinantHelper &operator=(std::vector<char> blob);
